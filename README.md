...
## Home Assistant custom integration (new)

This repository now also includes a Home Assistant custom integration:
...
4. Search for **Haier AC Bridge** and provide:
   - Bridge IP (`host`)
   - Bridge token (`token`)
   - Optional behavior settings (`polling`, `use_fan_mode`, `use_dry_mode`, `health_mode_type`, `swing_type`, naming fields)

### Entity mapping

- One `climate` entity per AC device.
- Optional `switch` entities based on configuration:
  - `Health Mode` (`health_mode_type=SHOW`)
  - `Dry Mode` (`use_dry_mode=true`)
  - `RightLeft Swing` + `UpDown Swing` (`swing_type=INDIVIDUAL`)
- `swing_type=BOTH` is exposed through climate swing mode.

### Legacy Homebridge code

The original Homebridge plugin code is still present in the root (`index.js`, `src/*`) for reference and backward compatibility.
